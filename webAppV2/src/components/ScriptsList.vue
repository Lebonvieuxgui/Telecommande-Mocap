                ...
                <tbody>
                    <el-checkbox-group v-model="selectedScripts">
                        <el-checkbox v-for="script in activeScripts" :key="script.name" :label="script.name"
                            ...
                            {{  script.name  }}<el-button size="small" text
                                @click="dialogFormVisible = true, selectScript(script.name, activeScripts)">
                                <el-icon>
                                    <Edit />
                                </el-icon>
                            </el-button>
                        ...
                        <el-dialog v-model="dialogFormVisible" title="Edit Script">
                            <el-form :model="form">
                                ...
                                <el-form-item class="DialogScript" label="Executable" :label-width="formLabelWidth">
                                    <el-select v-model="form.executableName"
                                        placeholder="Please select an executable name">
                                        <el-option v-for="loadedExec in loadedExecs" :key="loadedExec.name"
                                            :value="loadedExec.name" :label="loadedExec.name" />
                                    </el-select>
                                </el-form-item>
                                <el-form-item class="DialogScript" label="Start Command Line Arguments"
                                    :label-width="formLabelWidth">
                                    <el-input v-model="form.startArgs" type="textarea" :rows="1" autocomplete="off" />
                                </el-form-item>
                            </el-form>
                            <el-form-item class="DialogScript" label="Stop Command Line Arguments"
                                :label-width="formLabelWidth">
                            ...
                            <template #footer>
                                <span class="dialog-footer">
                                    ...
                                    <el-button type="primary" @click="dialogFormVisible = false, postChanges()">
                                        Confirm
                                    </el-button>
                                </span>
                            </template>
    ...
    methods: {
        ...
        selectScript(name, activeScripts) {
            ...
            if (this.form.name === null) {
                form = { name: undefined, executableName: undefined, startArgs: undefined, stopArgs: undefined };
            }
        },
        postChanges() {
